...
.mine_header_content_share_iteml {
    float: left;
    width: 120rpx;
    height: 60rpx;
    text-align: center;
...
.mine_header_content_share_itemr {
    float: right;
    width: 120rpx;
    height: 60rpx;
    line-height: 60rpx;
...
<template>
    <div class="mine">
        ...
        <div class="mine_content">
            ...
            <div class="mine_menu_current" v-for="(item,key) in myself.article" :key="key" @click="particularapi(item.id)">
...
export default {
    ...
    methods:{
        ...
        dotattention (id) {
            ...
            .then(function (res) {
                if(res.success) {
                    ...
                    _this.myself.g_type = res.data.data.g_type
                }else {
                    wx.showToast({
                        title: '失败',
                        icon: 'success',
        ...
        particularapi (id) {
            const url = '../indexitem/main?id=' + id
            wx.navigateTo({ url })
        },
        // 打赏详情
        exceptionaldetails () {
        ...
        personage () {
            var _this = this
            var opid = wx.getStorageSync('openid')
            this.$get('/restapi/userinfo/select',{
                openid: this.$mp.query.opid,
                use_openid: opid
            })
            .then(function (res) {
                if(res.success) {
